...
export class LotService {
      ...
      for (const itemData of data.items) {
        ...
        if (itemData.itemType === LotItemType.MODEL) {
          ...
          if (!itemData.sparePartId)
            throw new AppError('Spare Part ID required for Spare Part item', 400);
  ...
  async processExcelUpload(buffer: Buffer): Promise<Lot> {
    ...
    const itemsData = XLSX.utils.sheet_to_json(worksheet, { range: 15 }) as Record<
      string,
      string | number
    >[]; // 0-indexed, so Row 16
    ...
    for (const row of itemsData) {
      ...
      if (itemType === LotItemType.MODEL) {
        ...
        const sparePart = await Source.getRepository(SparePart)
  ...
  async generateSparePartsExcel(lotId: string): Promise<Buffer> {
    ...
    // Header row
    rows.push(['item_code', 'part_name', 'brand', 'model_id', 'base_price', 'quantity', 'lot_id']);

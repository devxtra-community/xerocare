...
export class LotService {
        ...
        return await Source.transaction(async (manager: EntityManager) => {
            ...
            for (const itemData of data.items) {
                ...
                if (itemData.itemType === LotItemType.MODEL) {
                    ...
                    if (!itemData.sparePartId) throw new AppError('Spare Part ID required for Spare Part item', 400);
    ...
    async generateExcel(lotId: string): Promise<Buffer> {
        ...
        // 3. Items
        rows.push(['LOT ITEMS']);
        const itemHeaders = ['Type', 'Name/Model', 'Brand', 'Quantity (Total)', 'Quantity (Used)', 'Remaining', 'Unit Price', 'Total Price'];
        rows.push(itemHeaders);

        lot.items.forEach(item => {
            const name = item.itemType === LotItemType.MODEL ? item.model?.model_name : item.sparePart?.part_name;
            const brand = item.itemType === LotItemType.MODEL ? item.model?.brandRelation?.name : item.sparePart?.brand;
    ...
    async processExcelUpload(buffer: Buffer): Promise<Lot> {
        ...
        if (!vendorName || !lotNumber || !purchaseDateStr) {
            throw new AppError('Missing Lot Information (Vendor, Lot Number, or Purchase Date). Please use the provided template.', 400);
        }
        ...
        const itemsData: any[] = XLSX.utils.sheet_to_json(worksheet, { range: 15 }); // 0-indexed, so Row 16
        ...
        const vendor = await Source.getRepository(Vendor).findOne({ where: { name: String(vendorName).trim() } });
        if (!vendor) {
        ...
        for (const row of itemsData) {
            const itemTypeRaw = row['Item Type']?.toString().trim().toUpperCase();
            const itemName = row['Item Name']?.toString().trim();
            const brandFromExcel = row['Brand']?.toString().trim(); // Read brand from Excel
            ...
            if (!itemTypeRaw || !itemName || isNaN(quantity) || isNaN(unitPrice)) {
                throw new AppError(`Invalid data in row for item '${itemName || 'Unknown'}'. Check Type, Name, Qty, Price.`, 400);
            }

            let itemType: LotItemType;
            if (itemTypeRaw === 'PRODUCT' || itemTypeRaw === 'MODEL') itemType = LotItemType.MODEL;
            else if (itemTypeRaw === 'SPARE PART' || itemTypeRaw === 'SPARE_PART') itemType = LotItemType.SPARE_PART;
            ...
            if (itemType === LotItemType.MODEL) {
                const model = await Source.getRepository(Model).createQueryBuilder('model')
                ...
                if (brandFromExcel && model.brandRelation?.name) {
                    if (brandFromExcel.toLowerCase() !== model.brandRelation.name.toLowerCase()) {
                        console.warn(`Brand mismatch for ${itemName}: Excel='${brandFromExcel}', Model='${model.brandRelation.name}'. Using Excel value.`);
                    }
                }

                itemDto.modelId = model.id;
            } else {
                const sparePart = await Source.getRepository(SparePart).createQueryBuilder('sp')
                    .where('LOWER(sp.part_name) = LOWER(:name)', { name: itemName })
                    .getOne();

                if (!sparePart) throw new AppError(`Spare Part '${itemName}' not found`, 404);

                // Use brand from Excel if provided, otherwise auto-fill from spare part
                if (brandFromExcel && sparePart.brand) {
                    if (brandFromExcel.toLowerCase() !== sparePart.brand.toLowerCase()) {
                        console.warn(`Brand mismatch for ${itemName}: Excel='${brandFromExcel}', SparePart='${sparePart.brand}'. Using Excel value.`);
                    }
                }
    ...
    async generateSparePartsExcel(lotId: string): Promise<Buffer> {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, `SpareParts-${lot.lotNumber.substring(0, 20)}`);

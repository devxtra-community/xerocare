...
function CashFlowContent() {
  ...
  const accountParams = branchIds ? { branchIds } : {};
  const cashbookParams = branchIds ? { branchIds, toDate: to } : { toDate: to };

  const { data: accounts = [], isLoading: loadingAccounts } = useQuery({
    queryKey: ['admin-cf-accounts', branchIds],
    queryFn: () => fetchCashBankAccounts(accountParams),
    staleTime: 60_000,
  });
  ...
  const {
    ...
    queryKey: ['admin-cf-cashbook-all', branchIds, to],
    queryFn: () => fetchCashbookEntries(cashbookParams),
    staleTime: 60_000,
    enabled: !!to,
  });

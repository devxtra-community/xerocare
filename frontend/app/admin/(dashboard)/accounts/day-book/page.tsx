...
import { BranchActivityView } from '@/components/accounts/BranchActivityView';
...
function DayBookContent() {
  ...
  const params: Record<string, string> = { fromDate: from, toDate: to };
  ...
  const {
    ...
    queryKey: ['admin-daybook', branchIds, from, to],
  ...
  const reverseMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin-daybook'] });
    },
    onError: (err: unknown) => {
      ...
      <Tabs defaultValue="cash" className="w-full space-y-4">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-xs text-muted-foreground">
                ...
                expenses). Entries are auto-posted from invoice receipts and expense payments, plus
                any manual cashbook entries.
              </p>
            </>
          )}
        </TabsContent>
        ...
        <TabsContent value="activity" className="space-y-6">
          {!isSingleBranch ? (
            ...
            <BranchActivityView branchId={branchIds} />
          )}
        </TabsContent>
      </Tabs>
    </div>

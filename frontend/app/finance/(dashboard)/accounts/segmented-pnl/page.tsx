...
// ─── One segment row — shared by Sale / Rent / Lease tables ────────────────────
function SegmentRow({
  seg,
  currency,
  onViewSource,
...
export default function SegmentedPnlPage() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['segmented-pnl', periodFrom, periodTo],
    queryFn: () => fetchSegmentedPnl({ periodFrom, periodTo }),
    staleTime: 60_000,
  });

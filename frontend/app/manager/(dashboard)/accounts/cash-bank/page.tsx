...
export default function ManagerCashBankPage() {
  ...
  const exportExcel = () => {
    if (activeTab === 'accounts') {
      ...
      XLSX.writeFile(wb, 'cash_bank_accounts.xlsx');
    } else {
      const ws = XLSX.utils.json_to_sheet(
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Eye className="h-6 w-6 text-blue-600" /> Cash & Bank
          </h1>
          <p className="text-sm text-gray-500">Your branch accounts — view only</p>
        </div>
        ...
        <div className="flex border-b">
          {(['accounts', 'cashbook'] as const).map((tab) => (
            <button
              ...
              className={`px-6 py-3 text-sm font-medium transition-colors ${activeTab === tab ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
        ...
        {activeTab === 'accounts' ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs uppercase text-gray-500">
                <tr>
                  {['Reference', 'Date', 'Type', 'Description', 'Account', 'Amount'].map((h) => (
                    <th key={h} className="px-4 py-3 text-left font-medium">
                      {h}
                    </th>
                  ))}

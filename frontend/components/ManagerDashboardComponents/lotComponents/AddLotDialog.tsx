...
import { Plus, Trash2, Upload, FileSpreadsheet, Download, Loader2, Info } from 'lucide-react';
...
export default function AddLotDialog({ onClose, onSuccess }: AddLotDialogProps) {
    ...
    const form = useForm<CreateLotFormValues>({
        ...
        defaultValues: {
            ...
            notes: '',
            items: [{ itemType: LotItemType.MODEL, quantity: 1, unitPrice: 0, modelId: '', sparePartId: '' }],
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const msg = error.response?.data?.message || 'Failed to upload lot';
            toast.error(msg);
        } finally {
            setLoading(false);
        }
    ...
    const watchedCosts = form.watch([
        "transportationCost", "documentationCost", "shippingCost",
        "groundFieldCost", "certificationCost", "labourCost"
    ] as const);

    const calculateItemsTotal = () => {
        return watchedItems?.reduce((sum, item) => sum + (item.quantity || 0) * (item.unitPrice || 0), 0) ?? 0;
    };
    ...
    const formControl = form.control as unknown as Control<any>;

    return (
        <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
            <DialogContent showCloseButton={false} className="sm:max-w-[80vw] lg:max-w-7xl h-[90vh] flex flex-col p-0 overflow-hidden bg-white shadow-2xl">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-12 gap-8">
                        <div className="text-center space-y-4 max-w-lg">
                            ...
                            <p className="text-gray-500 text-base">
                                Download the template, fill in your lot details including items and costs, and upload it here.
                            </p>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl">
                            <div className="border rounded-lg p-6 hover:bg-gray-50 transition-colors cursor-pointer flex flex-col items-center text-center gap-3" onClick={handleDownloadTemplate}>
                                <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center">
                                    <Download className="w-6 h-6 text-gray-600" />
                                </div>
                                <div>
                                    <h4 className="font-semibold">Download Template</h4>
                                    <p className="text-xs text-gray-500 mt-1">Get the properly formatted Excel file</p>
                                </div>
                            </div>
                                ...
                                <div className="col-span-12 lg:col-span-9 flex flex-col h-full overflow-hidden border-r">
                                    <div className="p-6 pb-2 grid grid-cols-1 md:grid-cols-4 gap-6">
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem className="space-y-1.5">
                                                    <FormLabel className="text-sm font-medium text-gray-700">Lot / Order Number</FormLabel>
                                                    <FormControl>
                                    ...
                                    <div className="flex items-center justify-between px-6 py-4 mt-2">
                                        ...
                                        <Button
                                            ...
                                            size="sm"
                                            variant="outline"
                                            onClick={() => append({ itemType: LotItemType.MODEL, quantity: 1, unitPrice: 0, modelId: '', sparePartId: '' })}
                                        ...
                                        <div className="h-full border rounded-lg flex flex-col overflow-hidden bg-gray-50/50">
                                            ...
                                            <div className="flex-1 overflow-y-auto p-4 space-y-3">
                                                {fields.map((field, index) => (
                                                    <div key={field.id} className="grid grid-cols-12 gap-4 items-start bg-white p-4 rounded-lg border shadow-sm"
                                                    >
                                                        <div className="col-span-12 md:col-span-2">
                                                            <div className="text-xs font-semibold text-gray-500 mb-1.5 md:hidden">Type</div>
                                                            <FormField
                                                                control={formControl}
                                                                ...
                                                                render={({ field }) => (
                                                                    <Select
                                                                        onValueChange={(val) => {
                                                                            ...
                                                                            update(index, { ...currentItem, itemType: val as LotItemType, modelId: '', sparePartId: '' });
                                                                        }}
                                                                        value={field.value}
                                                                    >
                                                                        ...
                                                                        <SelectContent>
                                                                            ...
                                                                            <SelectItem value={LotItemType.SPARE_PART}>Spare Part</SelectItem>
                                                                        </SelectContent>
                                                                    </Select>
                                                                )}
                                                            />
                                                        ...
                                                        <div className="col-span-12 md:col-span-5">
                                                            <div className="text-xs font-semibold text-gray-500 mb-1.5 md:hidden">Item Details</div>
                                                            <FormField
                                                                control={formControl}
                                                                name={`items.${index}.itemType`}
                                                                render={({ field }) => {
                                                                            ...
                                                                            {type === LotItemType.MODEL ? (
                                                                                <FormField
                                                                                    ...
                                                                                    render={({ field: modelField }) => (
                                                                                        <Select onValueChange={modelField.onChange} value={modelField.value}>
                                                                                            <SelectTrigger className="h-10 text-sm bg-gray-50/50">
                                                        ...
                                                        <div className="col-span-6 md:col-span-2">
                                                            <div className="text-xs font-semibold text-gray-500 mb-1.5 md:hidden">Quantity</div>
                                                            <FormField
                                                                control={formControl}
                                                        ...
                                                        <div className="col-span-6 md:col-span-2">
                                                            <div className="text-xs font-semibold text-gray-500 mb-1.5 md:hidden">Unit Price</div>
                                                            <div className="relative">
                                                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">$</span>
                                                                <FormField
                                                                    control={formControl}
                                            ...
                                            <div className="bg-white border-t p-4 text-right flex justify-end items-center gap-2">
                                                ...
                                                <span className="font-bold text-lg text-gray-900">${calculateItemsTotal().toFixed(2)}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="col-span-12 lg:col-span-3 flex flex-col h-full overflow-hidden bg-white">
                                    ...
                                    <div className="flex-1 overflow-y-auto px-6 py-2">
                                        <div className="space-y-5">
                                            <CostInput control={formControl} name="transportationCost" label="Transportation" />
                                            <CostInput control={formControl} name="documentationCost" label="Documentation" />
                                            <CostInput control={formControl} name="shippingCost" label="Shipping" />
                                            <CostInput control={formControl} name="groundFieldCost" label="Ground / Field" />
                                            <CostInput control={formControl} name="certificationCost" label="Certification" />
                                    ...
                                    <div className="p-6 border-t mt-auto">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className="font-bold text-base text-gray-900">${calculateCostsTotal().toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>

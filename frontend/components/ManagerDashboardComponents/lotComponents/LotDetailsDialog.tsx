                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-sm text-gray-500">
                            Vendor: <span className="font-medium text-foreground">{lot.vendor?.name}</span> • Date:{' '}
                            {format(new Date(lot.purchaseDate), 'PPP')}
                        </p>
                    </div>
                    <div className="flex gap-2">
                        ...
                        {hasSpareParts && (
                            <Button variant="outline" size="sm" onClick={handleExportSpareParts} className="gap-2">
                ...
                <div className="flex gap-6 overflow-hidden flex-1">
                                ...
                                <TableBody>
                                    {lot.items.map((item) => (
                                        <TableRow key={item.id}>
                                            ...
                                            <TableCell className="max-w-[200px] truncate" title={item.itemType === LotItemType.MODEL ? item.model?.model_name : item.sparePart?.part_name}>
                                                {item.itemType === LotItemType.MODEL
                                                    ? `${item.model?.model_name} (${item.model?.model_no})`
                                                    : item.sparePart?.part_name}
                                            </TableCell>
                                            <TableCell className="text-right">{item.quantity}</TableCell>
                                            <TableCell className="text-right text-orange-600">{item.usedQuantity}</TableCell>
                                            <TableCell className="text-right font-medium text-green-600">{item.quantity - item.usedQuantity}</TableCell>
                    ...
                    <div className="flex-1 border rounded-lg bg-slate-50 flex flex-col">
                        ...
                        <div className="bg-blue-50 border-t p-4 flex justify-between items-center">
                            ...
                            <span className="font-bold text-xl">${Number(lot.totalAmount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                        </div>
                    </div>
                </div>
            </div>

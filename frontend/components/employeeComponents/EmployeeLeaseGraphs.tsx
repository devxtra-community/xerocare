...
const CustomTooltip = ({ active, payload, label }: TooltipProps<ValueType, NameType>) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-3 rounded-xl shadow-lg border border-blue-100 min-w-[120px]">
...
export default function EmployeeLeaseGraphs({ invoices: propInvoices }: EmployeeLeaseGraphsProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Failed to fetch lease graph data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-4 pb-6">
      <h3 className="text-lg sm:text-xl font-bold text-primary">Lease Analytics</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Lease Per Month */}
        <ChartCard title="Lease Per Month">
        ...
        {/* Lease Per Day */}
        <ChartCard title="Lease Per Day (Current Month)">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={dailyData} margin={{ top: 5, right: 5, left: -30, bottom: 5 }}>
              <defs>
                <linearGradient id="colorCountLease" x1="0" y1="0" x2="0" y2="1">

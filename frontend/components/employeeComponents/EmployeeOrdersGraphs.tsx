...
interface EmployeeOrdersGraphsProps {
  invoices?: Invoice[];
}
...
export default function EmployeeOrdersGraphs({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Failed to fetch order graph data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-4 pb-6">
      <h3 className="text-lg sm:text-xl font-bold text-primary">Orders Analytics</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Orders Per Day */}
        <ChartCard title="Orders Per Day (Current Month)">
          <ResponsiveContainer width="100%" height="100%">

...
export function QuotationViewDialog({
  ...
  */
  ...
  /**
   * Warranty, as data handed to whichever template is rendering.
   *
   * It used to be drawn by this dialog after the template, which put it below that
   * document's own letterhead footer — outside the page. The templates own their layout, so
   * the section belongs to them; this only supplies the values.
   */
  const templateWarranty = {
      ...
      {isReturnInvoice && !showingOriginalInvoice ? (
        ...
        <DialogContent className="sm:max-w-5xl rounded-none border-none shadow-sm p-0 overflow-hidden bg-white flex flex-col max-h-[95vh]">
          ...
          <div id="quotation-print-content" className="flex-1 overflow-y-auto scrollbar-hide">
            ...
            {/* The warranty section now lives INSIDE each template (see
                WarrantyDetailsBlock). It used to be rendered here, after the template
                component — which placed it below that document's own letterhead footer,
                outside the page. The lease templates had always drawn their own, so lease

...
import { Lot, LotItem, lotService } from '@/lib/lot';
  ...
  useEffect(() => {
    const loadDependencies = async () => {
      ...
      if (wResult.status === 'fulfilled') {
        setWarehouses(wResult.value);
      } else {
        console.warn('Failed to load warehouses by branch, falling back to all warehouses...');
        try {
          const allWh = await commonService.getAllWarehouses();
          setWarehouses(allWh);
        } catch (err) {
          console.error('Failed to load all warehouses:', err);
        }
      }

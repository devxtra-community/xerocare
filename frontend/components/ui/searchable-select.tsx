...
export function SearchableSelect({
  ...
  const filteredOptions = options.filter((option) => {
    ...
    // searchText first — it exists precisely because `label` is often a React node, and
    // String(<span/>) is "[object Object]", which matches nothing a user would ever type.
    // The filter used to skip it entirely, so every picker with a styled label (products
    // with a status chip, machines with a serial) silently returned "No results found"
    // for any search at all: the option was there, it just could never be matched.
    const searchableStr = option.searchText ? String(option.searchText).toLowerCase() : '';
    ...
    return (
      (searchableStr && searchableStr.includes(searchStr)) ||
      (labelStr && labelStr.includes(searchStr)) ||
